<template>
  <div class="seller-wrapper">
    ...
    <div class="rank">
      <p class="title" ref="rankTitle">导购排名</p>
      <template>
        <cube-tab-bar v-model="selectedLabel" show-slider ref="cubeTab">
              ...
              <cube-scroll
                ...
                <ul v-if="item.children.length">
                  <li v-for="(item, index) in item.children" :key="index" class="sell-item">
                    <p>
                      <span class="item-num">{{index + 1}}</span><span class="avatar"></span><span>{{item.staffName}}</span>
                    </p>
                    <p>
...
import { getStoreCount } from "@/services/store_rank";
import { getSellerList } from "@/services/seller_rank";
...
export default {
  ...
  computed:{
    ...mapGetters(['bottomTabHeight'])
  },
  mounted(){
    for (let i = 0 ; i < this.$refs.scrollDom.length; i ++) {
      this.$refs.scrollDom[i].style.height = (window.innerHeight - this.$refs.pageTop.clientHeight - this.$refs.rankTitle.clientHeight - this.$refs.cubeTab.$el.clientHeight - this.bottomTabHeight) + 'px';
    }
    this.getStoreCountMsg();
    this.getSellerListMsg('saleAmount');
    this.getSellerListMsg('repeat');
    this.getSellerListMsg('atv');
    this.getSellerListMsg('recruit');
  },
  methods:{
    getStoreCountMsg() {
